...
def export_data_to_postgres(df: pd.DataFrame, *args, **kwargs) -> None:
    """
    Exporta el DataFrame de Items normalizados a Postgres RAW.
    ...
    # 🔑 Convertir payload (dict → JSON string)
    ...
    # Exportar: si la tabla no existe, se crea automáticamente
    df.to_sql(
        table_name,
        engine,
        schema=schema,
        if_exists="append",  # usa "replace" si quieres sobreescribir cada vez
        index=False,
        method="multi"
    )
